/*
* Classe Orderbook ha il compito di gestire il file orderbook
 */
package cross.server.orderbook;

import cross.server.order.*;
import cross.server.handler.StoricoOrdiniHandler;
import cross.server.utilities.NotificationSender;
import cross.server.utilities.Trade;
...
public class Orderbook {
    ...
    public int addLimitOrder(LimitOrder order) {
        ConcurrentSkipListMap<Integer, ConcurrentLinkedQueue<LimitOrder>> oppositeBook =
                order.getType().equals("bid") ? sellOrders : buyOrders;

        List<Trade> executedTrades = new ArrayList<>();
        int remainingSize = order.getSize();

        // Condizione di compatibilità prezzi
        boolean isBid = order.getType().equals("bid");
        Iterator<Map.Entry<Integer, ConcurrentLinkedQueue<LimitOrder>>> it =
                isBid ? oppositeBook.entrySet().iterator()
                        : oppositeBook.descendingMap().entrySet().iterator();

        while (it.hasNext() && remainingSize > 0) {
            Map.Entry<Integer, ConcurrentLinkedQueue<LimitOrder>> entry = it.next();
            int matchPrice = entry.getKey();

            // Condizione di prezzo
            if ((isBid && matchPrice > order.getPriceLimitOrder()) ||
                    (!isBid && matchPrice < order.getPriceLimitOrder())) {
                break;
            }

            ConcurrentLinkedQueue<LimitOrder> queue = entry.getValue();
            Iterator<LimitOrder> queueIt = queue.iterator();

            while (queueIt.hasNext() && remainingSize > 0) {
                LimitOrder opp = queueIt.next();
                int tradeSize = Math.min(remainingSize, opp.getSize());

                // Crea trade ed eventuale notifica
                Trade trade1 = new Trade(order.getId(), order.getType(), "limit", tradeSize, matchPrice);
                Trade trade2 = new Trade(opp.getId(), opp.getType(), "limit", tradeSize, matchPrice);
                List<Trade> list1 = new ArrayList<>();
                list1.add(trade1);
                sender.sendNotification(list1, order.getIP(), order.getPort());

                List<Trade> list2 = new ArrayList<>();
                list2.add(trade2);
                sender.sendNotification(list2, opp.getIP(), opp.getPort());
                executedTrades.add(trade1);
                executedTrades.add(trade2);

                remainingSize -= tradeSize;
                opp.setSize(opp.getSize() - tradeSize);

                if (opp.getSize() == 0) queueIt.remove();
            }

            if (queue.isEmpty()) it.remove();
        }

        // Se rimane qualcosa, lo inserisco nel book
        if (remainingSize > 0) {
            order.setSize(remainingSize);
            ConcurrentSkipListMap<Integer, ConcurrentLinkedQueue<LimitOrder>> myBook =
                    order.getType().equals("bid") ? buyOrders : sellOrders;
            myBook.computeIfAbsent(order.getPriceLimitOrder(), k -> new ConcurrentLinkedQueue<>()).add(order);
        }
            ...
            for (Map.Entry<Integer, ConcurrentLinkedQueue<LimitOrder>> entry : map.entrySet()) {
                ...
                while (iterator.hasNext()) {
                    ...
                    if (limitOrder.getSize() < size) {
                        ...
                        size -= limitOrder.getSize();
                        Trade n1 = new Trade(limitOrder.getId(), limitOrder.getType(), limitOrder.getOrderType(), limitOrder.getSize(), limitOrder.getPriceLimitOrder());
                        Trade n2 = new Trade(order.getId(), order.getType(), order.getOrderType(), limitOrder.getSize(), limitOrder.getPriceLimitOrder());
                        trades.add(n1);
                        ...
                        Trade n1 = new Trade(limitOrder.getId(), limitOrder.getType(), limitOrder.getOrderType(), size, limitOrder.getPriceLimitOrder());
                        Trade n2 = new Trade(order.getId(), order.getType(), order.getOrderType(), size, limitOrder.getPriceLimitOrder());
                        trades.add(n1);
                        List<Trade> temp = new ArrayList<>();
                        temp.add(n2);
                        System.out.println("LIMIT ORDER: " + limitOrder.getId());
    ...
    public void executeStopOrder(StopOrder order) {
        ...
        synchronized (map) {
            ...
            if (order.getType().equals("bid") && map.firstKey() >= order.getPriceStopOrder()) {
            ...
            if (order.getType().equals("ask") && map.firstKey() <= order.getPriceStopOrder()) {
