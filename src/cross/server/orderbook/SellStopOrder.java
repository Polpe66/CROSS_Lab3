/*
* Classe per eseguire gli ordini che sono di vendita di StopOrder
 */
...
public class SellStopOrder implements Runnable{
    ...
    public SellStopOrder(Orderbook orderbook, AtomicInteger price) {
    ...
    public int addSellStopOrder(StopOrder order) {
        ...
        synchronized (lock) {
            sellOrders.computeIfAbsent(order.getPriceStopOrder(), k -> new ConcurrentLinkedQueue<>()).add(order);
            lock.notifyAll();
        }
        return order.getId();
    }
